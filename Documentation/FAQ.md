...
A: The BETA state is meant to communicate that the list is still considered work-in-progress feature and design wise. The label does not mean the list is unsuitable for longer playthroughs or that it is inherently unstable. The BETA phase ends when I decide the intended gameplay experience and vision for the list is achieved.
